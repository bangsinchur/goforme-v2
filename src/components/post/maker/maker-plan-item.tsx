...
export default function MakerPlanItem({ id }: { id: string }) {
        ...
        <div className="flex flex-col gap-3 border-3 border-line-muted-foreground shadow-md rounded-md p-4">
            <div className="flex flex-col gap-1 border-b border-line-muted-foreground pb-2">
                <div className="flex justify-between items-center">
                    ...
                    <div className="text-xs text-muted-foreground">{formatTimeAgo(planPost.createdAt)}</div>
                </div>
                <div className="text-sm text-muted-foreground">{planPost.dreamer.nickName}</div>
            </div>
            <div className="flex flex-col gap-2">
                <div className="flex gap-4 items-center">
                    <div className="bg-muted rounded-md p-1">여행일</div>
                    <div className="text-lg">{formatToDetailedDate(planPost.tripDate)}</div>
                </div>
                <div className="flex gap-4 items-center">
                    <div className="bg-muted rounded-md p-1">여행지</div>
                    <div className="text-lg">{convertRegionToKorean(planPost.serviceArea)}</div>
                </div>
            </div>
            <Estimate planId={id} />
        </div>
